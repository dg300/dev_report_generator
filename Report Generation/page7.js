...
const page7 = async (doc,cosmosActiveDev) => {
    doc
    .addPage()
    .font('Helvetica')
    .fontSize(20)
    .text('Cosmos Weekly Active Developers', {
        align:'left'
    });
    ...
    .image(buff, {
      ...
      for(var i = 0;i < Object.keys(cosmosActiveDev).length-1;i++){
        week = Object.keys(cosmosActiveDev)[i];
        // if(new Date(week*1000).getM == new Date()) continue;
        avDev += cosmosActiveDev[week];
        if(cosmosActiveDev[week] > maxDev){
            maxDevWeek = week;
            maxDev = cosmosActiveDev[week];
        }
        if(cosmosActiveDev[week] < minDev){
            minDevWeek = week;
            minDev = cosmosActiveDev[week];
        }
    }
