...
// import ethereumCommitsChart from '../Chart Generation/ethereumCommitsChart.js';
import commitData from '../Chart Generation/commitData.js';
import cryptoChart from '../Chart Generation/cryptoChart.js';


const page2 = async (doc,ethereumRaw) => {
    doc
    .addPage()
    // .rect(0, 0, doc.page.width, doc.page.height).fill('#302ed6')
    .font('Helvetica')
    .fontSize(20)
    // .fillColor('green')
    .text('Ethereum Weekly Commit trends on Github', {

        align:'left'
    });
    doc
    // const buff = await ethereumCommitsChart(ethereumRaw);
    const buff = await cryptoChart(ethereumRaw,"Ethereum","Weekly Commit Data trends","#commits");
    doc
    .moveDown(3)
    .image(buff, {
        width:500,
    ...
    const weeklyData = commitData(ethereumRaw);
    ...
    .fontSize(18)
    .moveDown(1)
    .text("Key Points from Ethereum Commit trends for last 12 months")
    .moveDown(1)
    .fontSize(15)
    .list(points);
}
