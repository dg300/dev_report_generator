...
import commitData from './commitData.js';


const commitChartCombined = async (bitcoin, ethereum, cosmos) => {
    ...
    let labels = []; 
    Object.keys(bitcoinMonthly).forEach(week => {
        let date = new Date(week*1000);
        let str = `${date.getMonth()+1}/${date.getFullYear()}`;
        labels.push(str);
    }); 
    // for(let i =0;i < 52;i++) console.log(Object.values(cosmosMonthly)[i]);
    var cos = Object.values(cosmosMonthly);
    const line_chart = ChartJSImage().chart({
            ...
            "datasets": [
                {
                    ...
                    "data": Object.values(bitcoinMonthly),
                },
                {
                    "borderWidth": "2",
                    "label": "Ethereum",
                    ...
                    "data": Object.values(ethereumMonthly)
                },
                {
                    "borderWidth": "2",
                    "label": "Cosmos",
        ...
        "options": {
            ...
            "title": {
                "display": true,
                "text": "Weekly Commit Data trends"
            },
            "scales": {
                "xAxes": [
                    {
    ...
    const buff = await line_chart.toBuffer();
    return buff;
}

export default commitChartCombined;